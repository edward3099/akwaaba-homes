...
async function requireAgent(supabase: any) {
  ...
  console.log('Profile check - userProfile:', userProfile, 'error:', profileError)
  if (!userProfile || !['agent', 'developer'].includes(userProfile.user_role)) {
    console.log('Profile check failed - userProfile:', userProfile, 'user_role:', userProfile?.user_role)
